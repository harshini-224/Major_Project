...
async def lifespan(app: FastAPI):
    ...
    # CRITICAL RENDER CONFIGURATION: Use Render's external URL for Twilio webhooks
    # If not found (e.g., local test), default to localhost.
    ...
    # 4. Stop Scheduler on Shutdown
    scheduler.shutdown()
    print("Backend shutting down.")

app = FastAPI(lifespan=lifespan)

# --- 2) CLINICAL KNOWLEDGE BASE & IVR HELPERS (NO CHANGES) ---
CLINICAL_QUESTIONS = {
...
def normalize_phone_number(phone):
    ...
    # 2. If it already starts with '+91', it's good (13 characters)
    if clean_phone.startswith('+91') and len(clean_phone) == 13:
        return clean_phone

    # 3. Handle numbers starting with '91' but missing the '+' (12 characters)
    if clean_phone.startswith('91') and len(clean_phone) == 12:
        return f"+{clean_phone}"

    # 4. Handle 10-digit numbers (assuming the +91 prefix) (10 characters)
...
def get_patient_context(phone_number):
    """Looks up patient by phone number after normalizing the Twilio-provided number."""
    normalized_caller = normalize_phone_number(phone_number)
    
    for pid, context in PATIENTS_DB.items():
        stored_phone = context.get("phone")
        if stored_phone == normalized_caller:
            return pid, context
            
    for pid, context in PATIENTS_DB.items():
        if normalize_phone_number(context.get("phone")) == normalized_caller:
            return pid, context
...
# --- 4) IVR Endpoints (Twilio Webhooks) (NO CHANGES) ---
@app.post("/twilio")
...
# --- 5) Patient Enrollment Endpoints (APIs for Streamlit) (NO CHANGES) ---
...
async def add_new_patient_api(
    ...
    if any(normalize_phone_number(p['phone']) == normalized_phone for p in PATIENTS_DB.values()):
...
# --- 6) Dashboard and Intervention Endpoints (APIs for Streamlit) (NO CHANGES) ---
...
# --- 7) Scheduling and Server Control (PUBLIC_URL usage) ---
def call_patient(patient_id, phone_number):
    global PUBLIC_URL
    try:
        # For Render deployment, PUBLIC_URL is the RENDER_EXTERNAL_URL
...
@app.get("/call_patient_manual/{patient_id}")
async def manual_call_patient(patient_id: str):
    patient = PATIENTS_DB.get(patient_id)
    if not patient:
         raise HTTPException(status_code=404, detail="Patient not found")
    call_patient(patient_id, patient['phone'])
